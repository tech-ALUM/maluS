...
- [ ] **authz** (`api/authz.py`): a global admin passes every review guard,
...
- [ ] **Delete any comment**: the retract route (`web/router.py`) allows an admin
...
- [ ] **Re-open submission** (new): `reopen_submission` service
...
- [ ] **GUI controls for admin**: the finding page shows the dispose form and the
...
- [ ] **Tests**: an admin with **no membership** can dispose, verify, reopen,
