...
- [ ] **Copy-link affordance** on the review page (and the Members page): copies
...
- [ ] **Reviewer landing CTA:** when the current user's role is `reviewer`, the
      review page shows a prominent banner + primary CTA ("Add your comments" →
      `/ui/reviews/{id}/edit-copy`), above the existing button row.
- [ ] **"To comment" cue** in the review list (`/ui/reviews`): reviews where the
...
- [ ] **Tests:** a reviewer sees the landing CTA and the list badge; the badge
